...
export var Format;
(function (Format) {
    Format[Format["jsonc"] = 0] = "jsonc";
    Format[Format["json5"] = 1] = "json5";
    Format[Format["hjson"] = 2] = "hjson";
    Format[Format["toml"] = 3] = "toml";
    Format[Format["yaml"] = 4] = "yaml";
    Format[Format["ini"] = 5] = "ini";
})(Format || (Format = {}));
...
N.B. to be parsed correctly one of the above file extensions must be used!`);
    ...
    for (const srcFile of argv.slice(2)) {
        ...
        FS.writeFileSync(`${basename}.json`, JSON.stringify(parsedConfig, undefined, 4));
        console.log('write jsonc');
        FS.writeFileSync(`${basename}.jsonc`, JSONC.jsonc.stringify(parsedConfig, undefined, 4));
        console.log('write json5');
        FS.writeFileSync(`${basename}.json5`, JSON5.stringify(parsedConfig, undefined, 4));
        console.log('write hjson');
        FS.writeFileSync(`${basename}.hjson`, HJSON.stringify(parsedConfig, { space: 4 }));
        console.log('write yaml');
        FS.writeFileSync(`${basename}.yaml`, YAML.stringify(parsedConfig));
        console.log('write ini');
        FS.writeFileSync(`${basename}.ini`, INI.stringify(parsedConfig));
        if (Array.isArray(parsedConfig)) {
            console.log(`\x1B[31mtoml doesn't support top-level arrays\x1B[0m`);
        }
        else {
            console.log('write toml');
            FS.writeFileSync(`${basename}.toml`, TOML.stringify(parsedConfig));
        }
    }
    return 0;
}
export function parse(format, configContent) {
    switch (format) {
        case 'json':
    ...
    for (const [filename, format] of Object.entries(files)) {
        if (FS.existsSync(filename)) {
            try {
                ...
                throw new Error(`Unable to parse ${filename} as ${format}: ${ex.message}`);
            }
        }
    }
    throw new Error(`No config files found`);
}
export async function loadConfigAsync(files) {
    for (const [filename, format] of Object.entries(files)) {
        if (FS.existsSync(filename)) {
